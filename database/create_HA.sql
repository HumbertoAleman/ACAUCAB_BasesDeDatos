...
CREATE TABLE Tipo_Cerveza (
    ...
    CONSTRAINT categorizado FOREIGN KEY (fk_tipo_cerv) REFERENCES Receta (cod_tipo_cerv),
);
...
CREATE TABLE Lugar_Tienda (
    ...
    CONSTRAINT localizado FOREIGN KEY (fk_luga_tien) REFERENCES Lugar_Tien (cod_luga_tien)
);
...
CREATE TABLE Miembro (
    ...
    CONSTRAINT proveniente FOREIGN KEY (fk_luga_2) REFERENCES Lugar (fk_luga_2)
);
...
CREATE TABLE Compra (
    ...
    CONSTRAINT realiza FOREIGN KEY (fk_miem) REFERENCES Miembro (cod_miem)
);
...
CREATE TABLE Usuario (
    ...
    CONSTRAINT adaptado FOREIGN KEY (fk_clie) REFERENCES Cliente (cod_clie),
    CONSTRAINT convertido FOREIGN KEY (fk_miem) REFERENCES Miembro (cod_miem),
...
CREATE TABLE Telefono (
    ...
    CONSTRAINT utiliza FOREIGN KEY (fk_miem) REFERENCES Miembro (numeric),
...
CREATE TABLE Registro_Evento (
    ...
    CHECK ((fk_juez IS NOT NULL AND fk_clie IS NULL AND fk_miem IS NULL) OR (fk_juez IS NULL AND fk_clie IS NOT NULL AND fk_miem IS NULL) OR (fk_juez IS NULL AND fk_clie IS NULL AND fk_miem IS NOT NULL) OR),
);
...
CREATE TABLE DESC_CERV (
    ...
    PRIMARY KEY (fk_desc, fk_cerv_pres_1, fk_cerv_pres_1),
...
CREATE TABLE EMPL_CARG (
    ...
    CONSTRAINT contiene FOREIGN KEY (fk_depa) REFERENCES Departamento (cod_depa)
);
...
CREATE TABLE TIPO_CARA (
    ...
    CONSTRAINT estiliza FOREIGN KEY (fk_tipo_cerv) REFERENCES Tipo_Cerveza (cod_tipo_cerv),
    CONSTRAINT estilizada FOREIGN KEY (fk_cerv) REFERENCES Cerveza (cod_cerv),
);
...
CREATE TABLE CERV_PRES (
    ...
    CONSTRAINT empaquetada FOREIGN KEY (fk_pres) REFERENCES Cerveza (cod_pres),
...
CREATE TABLE ESTA_COMP (
    ...
    CONSTRAINT ensenado FOREIGN KEY (fk_esta) REFERENCES Estatus (fk_esta)
);
...
CREATE TABLE ESTA_EVEN (
    ...
    CONSTRAINT muestra FOREIGN KEY (fk_even) REFERENCES Estado (cod_even),
    CONSTRAINT mostrado FOREIGN KEY (fk_esta) REFERENCES Estatus (fk_esta)
);
...
CREATE TABLE ESTA_EVEN (
    ...
    CONSTRAINT reflejado FOREIGN KEY (fk_esta) REFERENCES Estatus (fk_esta)
);
...
CREATE TABLE PUNT_CLIE (
    ...
    CONSTRAINT guarda FOREIGN KEY (fk_clie) REFERENCES Cliente (cod_clie),
...
CREATE TABLE Tarjeta (
    ...
    FOREIGN KEY (fk_meto_pago) REFERENCES Metodo_Pago (fk_meto_pago)
);
