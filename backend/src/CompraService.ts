...
class CompraService {
	...
	@sqlProtection
	@LogFunctionExecution
	async setCompraPagada(compra: number) {
		const new_status = { fk_comp: compra, fk_esta: 4, fecha_ini: formatDate(new Date()) }; // HACK: FK_ROL = 4 es compra pagada
	...
	routes = {
		"/api/compra": {
			GET: async () => await this.getCompras()
		},
		"/api/set_compra_pagada": {
			POST: async (req: any) => this.setCompraPagada((await req.json()).fk_comp)
		}
	}
}
